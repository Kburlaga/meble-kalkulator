...
st.set_page_config(page_title="STOLARZPRO - V15 (PRO Dwustronna)", page_icon="🪚", layout="wide")
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    H_MEBLA = c1.number_input("Wys.", key='h_mebla')
    W_MEBLA = c2.number_input("Szer.", key='w_mebla')
    D_MEBLA = c1.number_input("Głęb.", key='d_mebla')
    GR_PLYTY = c2.number_input("Grubość", key='gr_plyty')
...
def dodaj_element(nazwa, szer, wys, gr, material, uwagi="", wiercenia=[], orientacja="L", strony_do_druku=None):
    ...
    lista_elementow.append({
        ...
        "strony_do_druku": strony_do_druku # Nowe pole dla przegród
    })
...
# Przegrody (Dwustronne)
for i in range(ilosc_przegrod):
    # Otwory dla strony LEWEJ przegrody (sekcja i)
    otwory_L = daj_otwory_dla_sekcji(konfiguracja[i]['typ'], konfiguracja[i]['ilosc'], 'L')
    # Otwory dla strony PRAWEJ przegrody (sekcja i+1)
    otwory_P = daj_otwory_dla_sekcji(konfiguracja[i+1]['typ'], konfiguracja[i+1]['ilosc'], 'L')
    
    wszystkie = otwory_L + otwory_P
    
    # Zapisujemy definicję dwóch stron do druku
    strony = [
        {'tytul': f"STRONA LEWA (Sekcja {i+1})", 'otwory': otwory_L},
        {'tytul': f"STRONA PRAWA (Sekcja {i+2})", 'otwory': otwory_P}
    ]
    
    dodaj_element("Przegroda", D_MEBLA, wys_wewnetrzna, GR_PLYTY, "", f"S{i+1}|S{i+2}", wszystkie, "L", strony_do_druku=strony)
...
for idx, konfig in enumerate(konfiguracja):
    ...
    if typ == "Szuflady":
        ...
        for sz in range(ilosc):
            dodaj_element("Front Szuflady", w_fr, h_frontu, 18, "", "", otw_front)
            dodaj_element("Dno Szuflady", dno_l, dno_w, 16, "", "")
            dodaj_element("Tył Szuflady", tyl_w, tyl_h, 16, "", "")
    elif typ == "Półka":
        dodaj_element("Półka", szer_jednej_wneki-2, D_MEBLA-20, 18, "", "", [])
...
with t2:
        ...
        if st.button("🚀 GENERUJ PDF", type="primary"):
            with st.spinner("Rysuję..."):
                ...
                with PdfPages(pdf_buffer) as pdf:
                    ...
                    for e in els:
                        # Jeśli element ma zdefiniowane strony (Przegroda), rysujemy je osobno
    ...
    if ids:
        ...
        # Jeśli przegroda - pokaż opcję wyboru strony
        if it.get('strony_do_druku'):
            strona_view = st.radio("Widok strony:", ["Lewa", "Prawa"], horizontal=True)
