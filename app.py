...
# ==========================================
# 1. FUNKCJA RYSUJĄCA (Z PRZESUNIĘTYMI ETYKIETAMI)
# ==========================================
def rysuj_element(szer, wys, id_elementu, nazwa, otwory=[], kolor_tla='#e6ccb3'):
    """
    Rysuje formatkę z otworami. Etykiety są przesunięte, aby nie zasłaniały otworów.
    """
    # Rozmiar wykresu
    ...
    for otw in otwory:
        ...
        # --- POPRAWKA CZYTELNOŚCI ---
        # Przesuwamy tekst: x+12, y+5 (żeby nie wchodził na kropkę)
        if len(otwory) < 40:
...
def optymalizuj_rozkroj(formatki, arkusz_w, arkusz_h, rzaz=4):
    # Sortowanie od największych elementów
    ...
    for f in formatki_sorted:
        ...
        # Czy mieści się w rzędzie?
        if cur_x + w + rzaz > arkusz_w:
            cur_x = 0
            cur_y += max_h_row + rzaz
            max_h_row = 0
            
        # Czy mieści się na wysokość arkusza?
...
wx = params["offset_front_x"] - axis_fuga
wy = params["offset_front_y"]
...
with t2:
    ...
    st.caption("Kliknij przycisk poniżej, aby wygenerować i pobrać komplet dokumentacji.")
    
    col_pdf1, col_pdf2 = st.columns([1, 2])
    with col_pdf1:
        if st.button("🚀 GENERUJ PLIK PDF", type="primary", use_container_width=True):
            with st.spinner("Składam dokumentację..."):
                pdf_buffer = io.BytesIO()
                with PdfPages(pdf_buffer) as pdf:
                    # Filtrowanie elementów (tylko te co mają wiercenia lub są frontami)
                    elements_to_print = [el for el in lista_elementow if el['wiercenia'] or el['Nazwa'] == 'Front Szuflady']
                    
                    if not elements_to_print:
                        st.warning("Brak elementów do wydruku.")
                    else:
    ...
    with col_pdf2:
        if 'pdf_ready' in st.session_state:
            ...
            st.download_button(
                ...
                file_name=f"{KOD_PROJEKTU}_Dokumentacja.pdf",
                mime="application/pdf",
                use_container_width=True
            )
        else:
            st.info("Oczekiwanie na wygenerowanie...")

    st.divider()
    
    st.subheader("👁️ Szybki Podgląd Elementu")
    ...
    if ids:
        ...
        st.info("Brak elementów do wyświetlenia.")
...
with t3:
    ...
    if st.button("Uruchom Optymalizację Rozkroju"):
        p18 = [x for x in lista_elementow if x['Materiał'] == "Płyta 18mm"]
        if not p18:
            st.warning("Brak formatek 18mm.")
        else:
            ...
            for i, ark in enumerate(wyniki):
                ...
                for el in ark['elementy']:
                    ...
                    # Podpis formatki
                    if el['w'] > 150:
                        ax.text(el['x']+el['w']/2, el['y']+el['h']/2, el['id'], ha='center', va='center', fontsize=7)
                
                ax.set_xlim(-100, ARKUSZ_W+100)
                ax.set_ylim(-100, ARKUSZ_H+100)
                ax.set_aspect('equal')
